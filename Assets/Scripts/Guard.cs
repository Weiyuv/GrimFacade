...
public class Guard : MonoBehaviour
{
    ...
    void MoveTowardsPlayer()
    {
        if (player != null)
        {
            ...
            if (distanceToPlayer > stoppingDistance)
            {
                ...
                // Rotaciona o inimigo para a direção do jogador
                if (direction.x < 0)
                {
    ...
    IEnumerator ShootAtPlayer()
    {
        ...
        // Calcula a direção do projétil, sem levar em consideração a rotação do firePoint
        Vector2 direction = (player.position - firePoint.position).normalized;
        
        // Cria o projétil na posição do ponto de disparo
        GameObject projectile = Instantiate(projectilePrefab, firePoint.position, Quaternion.identity);

        // Aplica a direção do projétil corretamente, sem ser afetado pela rotação do firePoint
