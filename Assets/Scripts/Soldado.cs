...
public class soldado : MonoBehaviour
{
    ...
    void MoveTowardsPlayer()
    {
        if (player != null)
        {
            ...
            if (distanceToPlayer > stoppingDistance)
            {
                ...
                // Ajusta o ponto de disparo conforme a direção do movimento
                if (direction.x < 0)
                {
                    firePoint.localRotation = Quaternion.Euler(0, 180, 0); // Gira o ponto de disparo para a esquerda
                }
                else
                {
                    firePoint.localRotation = Quaternion.Euler(0, 0, 0); // Gira o ponto de disparo para a direita
                }
            }
        }
    }
    ...
    IEnumerator ShootAtPlayer()
    {
        ...
        // Aplica a rotação dependendo da direção do ponto de disparo
        if (firePoint.localRotation.y == 180)
        {
            // Inverte a direção do projétil se o ponto de disparo estiver virado para a esquerda
